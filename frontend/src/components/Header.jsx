    ...
    <header className={`${location.pathname === '/' ? 'bg-transparent' : 'bg-back'} fixed top-0 z-20 flex items-center justify-between w-full px-12 py-5 text-sky`}>
      ...
      <nav className={`absolute ${active?'flex':'hidden'} right-11 top-20 bg-sky rounded-xl px-6 py-5`}>
        <ul className='flex flex-col justify-center items-center gap-6 text-xl font-semibold text-back '>
          <Link to="" className=' hover:border-b-[3px] hover:border-b-white transition-all '>INICIO</Link>
          <li><a href="#quienes_somos" className=' hover:border-b-[3px] hover:border-b-white transition-all '>QUIÉNES SOMOS</a></li>
          <li><a href="#buscar" className=' hover:border-b-[3px] hover:border-b-white transition-all '>BUSCAR</a></li>
          <Link to="catalogo" className=' hover:border-b-[3px] hover:border-b-white transition-all '>CATÁLOGO</Link>
        </ul>
      </nav>
